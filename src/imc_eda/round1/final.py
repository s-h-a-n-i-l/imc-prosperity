...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        self._update_total_pnl(memory, state, mids)
        ...
        if osmium_depth is not None:
            osmium_limit = self._osmium_effective_limit(memory, state, mids)
    ...
    def _trade_pepper(
        ...
        if best_bid > fair_value and position > 0:
            signal_side = -1
            quantity = self._sell_capacity(position, max_position)
            if quantity > 0:
                target_price = best_bid
                orders.append(Order(PEPPER, target_price, -quantity))
                action = "sell"
            ...
            signal_side = 1
            quantity = self._buy_capacity(position, max_position)
            if quantity > 0:
    ...
    def _load_memory(self, trader_data: str) -> dict[str, Any]:
        ...
        osmium_memory.setdefault("effective_limit", OSMIUM_BASE_LIMIT)
    ...
    def _default_memory(self) -> dict[str, Any]:
        return {
            ...
            "osmium": {
                "mid_history": [],
                "effective_limit": OSMIUM_BASE_LIMIT,
            },
            "portfolio": {
    ...
    def _osmium_effective_limit(
        ...
        portfolio = memory["portfolio"]
        osmium_cash = float(portfolio["cash"].get(OSMIUM, 0.0))
        osmium_position = float(state.position.get(OSMIUM, 0))
        osmium_mid = float(mids.get(OSMIUM, OSMIUM_REFERENCE_MID))
        osmium_marked_pnl = osmium_cash + osmium_position * osmium_mid

        if osmium_mid <= 0:
            return OSMIUM_BASE_LIMIT

        pnl_lots = int(math.floor(max(osmium_marked_pnl, 0.0) / osmium_mid))
        return max(OSMIUM_BASE_LIMIT, min(OSMIUM_EXCHANGE_LIMIT, OSMIUM_BASE_LIMIT + pnl_lots))
